...
class InterfaceApp:
    ...
    def __init__(self, master=None):
        ...
        self.appFrame.configure(height=650, width=850)
        ...
        self.algorithmbox.configure(cursor="hand2", state="readonly",
                                    values=('BFS', 'DFS (Graph Search)', 'DFS (Backtracking)', 'Voraz (Manhattan)',
                                            'ID', 'A* Manhattan', 'A* Euclidean', 'IDA* Manhattan', 'ExamenA', 'ExamenB'))
        ...
        self.computehcostbutton = ttk.Button(self.appFrame)
        self.computehcostbutton.configure(cursor="hand2", text='Compute H cost', state='normal')
        self.computehcostbutton.place(anchor="n", width=180, height=40, x=700, y=300)
        self.computehcostbutton.bind("<ButtonPress>", self.computeHCost)
        ...
        self.analysisbox.place(anchor="center", width=170, height=295, x=700, y=490)
        ...
        self.enterstatebutton.place(anchor="n", width=180, height=40, x=700, y=250)
    ...
    def computeHCost(self, event=None):
        ...
        if not initialState or not currentState:
            messagebox.showerror(message='Heuristic Cost', detail='No state to compute heuristic for')
        else:
            if algo in ['A* Manhattan', 'Voraz (Manhattan)', 'IDA* Manhattan']:
                ...
                result =  f'Euclidean distance: {main.getEuclideanDistance(currentState):.2f}'
            elif algo == 'MyMethod1':
                result = f'MyMethod1 heuristic: {main.getExamenA(currentState)}'
            elif algo == 'MyMethod2':
                result = f'MyMethod2 heuristic: {main.getExamenB(currentState)}'
            elif algo == 'ExamenA':
                result = f'ExamenA heuristic: {main.getExamenA(currentState)}'
            elif algo == 'ExamenB':
                result = f'ExamenB heuristic: {main.getExamenB(currentState)}'
            else:
    ...
    def enterInitialState(self, event=None):
        ...
        """
        global initialState, statepointer, currentState
    ...
    def fastForward(self, event=None):
        ...
        if statepointer < cost:
            app.stopbutton.configure(state='normal')
            statepointer += 1
            self.refreshFrame()
            ms = 100
            if 100 < cost <= 1000:
    ...
    def refreshFrame():
        ...
        if statepointer == 0:
            ...
            app.resetbutton.configure(state='normal')
            app.backbutton.configure(state='normal')
            app.fastbackwardbutton.configure(state='normal')

        if cost == 0 or statepointer == cost:
            app.fastforwardbutton.configure(state='disabled')
            app.nextbutton.configure(state='disabled')
        else:
            app.fastforwardbutton.configure(state='normal')
            app.nextbutton.configure(state='normal')
